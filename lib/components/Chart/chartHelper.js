...
var ChartHelper = /** @class */ (function (_super) {
    ...
    ChartHelper.prototype.shouldReCreateInstance = function (newConfig) {
        ...
        // scale 切换不需要重建实例
        var unCompareProps = __spreadArray(__spreadArray([], REACT_PIVATE_PROPS, true), ['scale', 'width', 'height', 'container', '_container', '_interactions', 'placeholder', /^on/, /^\_on/], false);
        if (!_isEqual(pickWithout(preOptions, __spreadArray([], unCompareProps, true)), pickWithout(options, __spreadArray([], unCompareProps, true)))) {
            return true;
        }
        return false;
    };
    ...
    ChartHelper.prototype.adapterOptions = function (_a) {
        ...
        // 剔除 React 自身的属性
        var options = pickWithout(others, __spreadArray([], REACT_PIVATE_PROPS, true));
        if (options.forceFit) {
